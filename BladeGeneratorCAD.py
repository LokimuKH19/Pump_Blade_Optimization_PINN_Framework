...
if __name__ == "__main__":
    ...
    # 叶片
    blade = BladeVoid(config["layers_params"], Theta, H, z0, hub_radius, shroud_radius, theta_offset=0.0)
    ...
    # ===== 创建叶片 =====
    print("Generating blade solid using intersection technique...")

    # 笑点解析：如果用辅助函数_summon_inner_hyposis_loft是可以的，但是四边形型面的质量太垃圾甚至没法做差，必须改成平滑的曲面
    # 生成叶片的型线
    ...
    # 预览上下型面
    write_step_file(upper_nurb, "./CQ/upper_surface.step")
    write_step_file(lower_nurb, "./CQ/lower_surface.step")
    # 生成前后缘线
    le_face, te_face = blade.build_le_te_faces()

    # 生成端盖
    print("Generating end caps via boolean operations...")
    hub_cap, shroud_cap = blade.to_cap()
    print(f"Hub cap generated: {not hub_cap.IsNull()}")
    print(f"Shroud cap generated: {not shroud_cap.IsNull()}")

    # 预览端盖
    write_step_file(hub_cap, "./CQ/hub_cap_boolean.step")
    write_step_file(shroud_cap, "./CQ/shroud_cap_boolean.step")

    # 直接合并所有部分创建完整的叶片实体，采用缝合
    print("Creating complete blade solid via sewing...")

    sewing = BRepBuilderAPI_Sewing(1e-4)    # 这个地方要想办法自适应
    ...
    print(f"Closed shell created: {not closed_shell.IsNull()}")
    # 预览壳体
    write_step_file(closed_shell, "./CQ/BladeShell.step")
    ...
    # 验证叶片实体
    blade_check = BRepCheck_Analyzer(complete_blade)
    print(f"Blade solid valid: {blade_check.IsValid()}")
    ...
    print("STEP files saved successfully.")
