...
### Data
- `./DataExample`: includes a .json file that defines the blade geometry (generated by [BladeOptimizerLFR](../BladeOptimizerLFR)) and the corresponding CFD simulation .csv file obtained by Ansys Fluent (shares the same operating parameters and model with the surrogate models), containing the information of nodes, position, velocity(in cartesian nates and the absolute pressure distribution). You can build your own data folder like this one (.json is necessary) and use them as data supervision during surrogate modeling.
- `./surrogate_formal`: typical running logs of the main program `SurrogateModeling.py`
- `./500EpochConvergenceTests`: evidences showing the effectiveness of the high-frequency strategies.\
- `./page`: new pages of the whole UI, for model deployment.
...
- `SurrogateDatasetPostprocess.py`: Hydraulic analysis of the CFD results. 
- `SurrogateDeployment.py`: Use the PINO to optimize the shape parameters of the blade via NSGA-II, based on the principle of an MOO problem: increasing efficiency/hydraulic head while simultaneously mitigating corrosion (lowering the characteristic flow speed).
