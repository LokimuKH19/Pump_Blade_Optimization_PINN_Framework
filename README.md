...
- `BladeOptimizerLFR` (Done): Allows parametric shape discription of an axial turbo machinery.
- `SurrogateModels` (Building): where the SIMPLE CFD data is applied to train physics-data hybrid driven surrogate models in dimensionless cylinderical coordinate, facilitating end-to-end flow field mapping from the blade definition file generated by `/BladeOptimizerLFR`.
- `BladeOptimizerMain` (Planned): The main workflow of optimizing the blade design with multiple objectives (Bezier Curvature + Spline Thickness) by using certain optimization algorithms (GAs, PSOs, Reinforcement learnings...) to obtain the Pareto front, where the target functions and constraints are calculated by real-time flow field surrogate models created from `SurrogateModels` to achieve acceleration x100~1000.
